...
use App\SypexGeo\Exceptions\UnknownFormatException;
use App\SypexGeo\Exceptions\NotFoundException;
...
class SxGeo
{
     ...
     */
	public function __construct(string $db_file = 'SxGeo.dat', int $type = SXGEO_FILE)
    {
		...
		if(substr($header, 0, 3) != 'SxG')
		    throw new NotFoundException("Can't open {$db_file}");
		...
		if($info['b_idx_len'] * $info['m_idx_len'] * $info['range'] * $info['db_items'] * $info['time'] * $info['id_len'] == 0)
		    throw new UnknownFormatException("Wrong file format {$db_file}");
